---
name: build-course1-statistics
description: "ARCHIVED — course1 is finished; this skill should not be invoked again. Kept for reference only. Formerly: build and iteratively improve course1, a Traditional Chinese introductory-statistics course from chapter Markdown notes through worked practice, cross-chapter method selection, and an interactive learning website, covering the Stanford/Coursera Introduction to Statistics curriculum under course_1/."
---

# [ARCHIVED] Build Course1: Introductory Statistics

> **Status: finished and archived.** All 12 course_1 chapters are complete through Phase 4; this skill is kept only as a historical reference for how course_1 was built. Do not invoke it again, and do not use it as a template for course_2 without checking `skills/build-course2-business-statistics/` first, since course_2 intentionally changes the phase-record design (see that skill's authoring standard).
...
4. If the course directories do not exist, run `python skills/build-course1-statistics/scripts/scaffold_course.py --root .`.
5. Read `course_1/進度.md` as the single source of truth. Work on the requested phase and chapter; if neither is specified, continue the earliest incomplete phase and module.
6. Update `course_1/進度.md` only after the relevant quality gate passes. Keep one row per chapter and record review evidence below the table.
...
Create one canonical Markdown file per module under `course_1/chapters/`. This same file is the living lecture note for all later phases; do not create separate Phase 2 or Phase 3 lecture-note copies. For every lesson topic:
...
Store reusable structured question data in `course_1/questions/<chapter>.yaml` when Phase 4 will render it interactively, but treat YAML as a rendering supplement, never the canonical lesson. The complete prompt, answer, misconception feedback, and step-by-step explanation must remain in the chapter Markdown.
...
Open each existing chapter Markdown and add cross-chapter comparisons and method-selection guidance in place. Do not create a separate Phase 3 version or move the explanations out of the chapter note. Also maintain `course_1/concept-map.md` and `course_1/method-selector.md` as course-wide indexes that link back to the canonical chapter sections.
...
- Generate figures with reproducible Python scripts under `course_1/figures/scripts/`, run them with `uv run`, and store outputs under `course_1/figures/generated/`.
...
Do not begin Phase 5 until every one of the 12 chapters is marked `完成` for Phase 1, Phase 2, Phase 3, and Phase 4 in `course_1/進度.md`. Scaffolding a web app, selecting a framework, or implementing shared site components also counts as beginning Phase 5 and must wait. When the gate opens, develop Phase 5 as one coordinated course-wide project rather than chapter by chapter.
...
- Use authoritative references to verify formulas when uncertain; record sources in `course_1/references.md`.
